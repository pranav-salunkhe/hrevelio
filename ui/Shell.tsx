...
import { WorkflowExecutor, WorkflowStatus } from '@io-orkes/conductor-javascript';
import { RingLoader } from './RingLoader';

// Interfaces
export interface Metrics {
    vocabulary: { value: number, impact: string },
    volume: { value: number, impact: string },
    difficulty: { value: number, impact: string },
    bugs: { value: number, impact: string },
    effort: { value: number, impact: string },
    length: { value: number, impact: string },
    critical: [{ fromLine: number, toLine: number, reason: string }]
}
...
interface WorkflowOutput {
    suggestionsResult?: { metrics: Metrics; };
    optimizedCodeResult?: { optimal: OptimizedCode; };
    testCasesResult?: { testCases: TestCase[]; };
}
...
export default function Shell() {
    ...
    // State for storing the results of each task
    ...
    const startWorkflow = async (workflowName: string, input: object) => {
        try {
    ...
    const handleGetSuggestions = () => {
        startWorkflow("CodeOptimizationWorkflow", { code });
    };

    // Handler for the "Optimize" button
    const handleOptimize = () => {
        startWorkflow("OptimizeCodeWorkflow", { code });
    };

    // Handler for the "Generate Unit Tests!" button
    const handleGenerateTestCases = () => {
        startWorkflow("GenerateTestCasesWorkflow", { optimizedCode: optimizedCode.optimizedCode });
    };
                    ...
                    <div className='overflow-y-hidden'>
                        <div className='flex w-full overflow-y-hidden gap-x-6'>
                            ...
                            <div className='flex flex-col w-full h-full justify-between gap-y-4'>
                                ...
                                <Button variant='light' disabled={loading} onClick={handleOptimize} radius={7} className='px-4 py-2'>
                                    {loading ? (
                                        <>
                    ...
                    <div className='overflow-y-hidden'>
                        <div className='flex w-full overflow-y-hidden gap-x-6'>
                            ...
                            <div className='flex flex-col w-[30%] h-full justify-between gap-y-4'>
                                ...
                                <Button variant='light' disabled={loading} onClick={handleGenerateTestCases} radius={7} className='px-4 py-2'>
                                    {loading ? (
                                        <>
                    ...
                    <div className='flex flex-col gap-y-6'>
                        ...
                        <div className='flex flex-col gap-y-6'>
                            ...
                            <div className='text-black opacity-60 font-light text-sm'>
                                <p>Pranav Salunkhe</p>
                                <ul>
                                    <li>National Institute of Technology Karnataka</li>
                                </ul>
                            </div>
                        </div>
                        <div className='flex flex-col gap-y-6'>
